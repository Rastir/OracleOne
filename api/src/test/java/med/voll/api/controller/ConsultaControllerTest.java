...
class ConsultaControllerTest {
    ...
    @Autowired
    private JacksonTester<DatosDetalleConsulta> datosDetallesConsultaJson;

    @MockitoBean
    private ReservaDeConsultas reservaDeConsultas;
    @Test
    @DisplayName("Deberia volver metodo http 400 cuando la request no tenga datos")
    @WithMockUser
    ...
    @Test
    @DisplayName("Deberia volver metodo http 200 cuando la request reciba un json valido")
    @WithMockUser
    void reservar_escenario2() throws Exception {

        var fecha = LocalDateTime.now().plusHours(1);
        var especialidad = Especialidad.CARDIOLOGIA;
        var datosDetalle = new DatosDetalleConsulta(null,1L,1L,fecha);
        ...
        var response = mvc.perform(post("/consultas")
                        ...
                        .content(datosReservaConsultaJson.write(
                                new DatosReservaConsulta(1L,1L,fecha,especialidad)
                        ).getJson()
                        )
                )
                .andReturn().getResponse();
        ...
        var jsonEsperado = datosDetallesConsultaJson.write(
                new DatosDetalleConsulta(null,1L,1L,fecha)
        ).getJson();
